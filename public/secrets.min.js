...
(function (root, factory) {
  ...
  function getShares(secret, numShares, threshold) {
    var shares = [],
      // coeffs는 다항식의 차수 저장하기 위한 배열
      coeffs = [secret],
      i,
      len;

    for (i = 1; i < threshold; i++) {
      // config.rng는 다항식 계수로 사용되는 난수 생성
  ...
  var secrets = {
    ...
    combine: function (shares, at) {
      ...
      for (i = 0, len = shares.length; i < len; i++) {
        ...
        console.log("combine loop ", i, share, setBits);
      ...
      for (i = 0, len = y.length; i < len; i++) {
        // Lagrange interpolation - 주어진 데이터 포인트들을 지나는 다항식을 찾는 방법을 통해 비밀 키를 복원함
      ...
      // step 3 -> 마커 1을 찾아서 데이터를 추출하고 최종 비밀 키를 반환
      console.log(
        bin2hex(at >= 1 ? result : result.slice(result.indexOf("1") + 1))
      );
    ...
    share: function (secret, numShares, threshold, padLength) {
      ...
      // step 2 -> 암호 생성 : 1을 secret 앞에 추가하고 정수 배열로 변환
      ...
      // splitNumStringToIntArray는 secret의 숫자 문자열의 길이를 맞춰줌 - 왼쪽에 0 추가
      secret = splitNumStringToIntArray(secret, padLength);

      // step 3 -> 비밀 값을 분해
      for (i = 0, len = secret.length; i < len; i++) {
        // numshare만큼의 subshare(부분적으로 나눈 비밀 값 - 계수, x,y값)을 생성
        subShares = getShares(secret[i], numShares, threshold);
        // x열, y열에 subshare 저장
        for (j = 0; j < numShares; j++) {
          // subshare의 x값을 변환하여 저장
          x[j] = x[j] || subShares[j].x.toString(config.radix);
          // 2진수 변환 후 왼쪽에 0을 채우고 이전 값과 합쳐서 저장
          y[j] = padLeft(subShares[j].y.toString(2)) + (y[j] || "");
        }
      }
      // x와 y배열은 각각 x좌표와 y좌표를 나타내는 문자열 배열이 됨

      // step 4 -> 생성된 비밀공유본을 문자열 형태로 변환
